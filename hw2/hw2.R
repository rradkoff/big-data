...
# reg5 <- glm(gt20dwn ~ ., data=as.data.frame(mm)[, names(signif)])
# print(summary(reg5))
...
reg5 <- glm(gt20dwn ~ .-AMMORT-LPRICE+FRSTHO*BATHS, data=homes[gt100,], family='binomial')
#reg4_fdr <- glm_fdr(reg4, q)
#print(summary(reg4_fdr))


#reg6 <- glm(gt20dwn ~ .-AMMORT-LPRICE+FRSTHO*BATHS, data=homes[gt100,], family='binomial')
#reg6 <- glm(gt20dwn ~.-AMMORT-LPRICE+FRSTHO*BATHS, 
            data=as.data.frame(mm)[gt100, names(signif)], family='binomial')
print(summary(reg6))
