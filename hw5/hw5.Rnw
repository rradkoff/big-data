...
most_common <- nroles[1]

degree <- degree(actnet)
degree <- degree[order(degree,decreasing=TRUE)]
most_connected <- degree[1]
@

We can determine the most common actors by summing up the columns of the actors' matrix and sorting the result. The actor appearing in the most films is \textbf{\Sexpr{names(most_common)}}, who appeared in \Sexpr{most_common} films. The most connected actor is \textbf{\Sexpr{names(most_connected)}}, who has been in at least one film with \Sexpr{most_connected} other actors.

<<shortest_path>>=
names <- colnames(actmat) 
cleanNames <- gsub("(.+),\\s+(.+)", "\\2 \\1", names)
cleanNames <- gsub("(.*)(\\(.+\\))(.*)", "\\1 \\3 \\2", cleanNames)
cleanNames <- gsub("\\s+", " ", cleanNames)
cleanMovies <- gsub('\\"', "", names(casts))
...
for ( k in 1:(length(spath)-1) ) { 
  ...
  common_movies <- c(common_movies, paste(cleanNames[spath[k]], "was in",
                                          paste("\\textit{",
                                                cleanMovies[intersect(m1, m2)][1],
                                                "}", sep=''),
                                          "with", cleanNames[spath[k+1]]))
}
@ 

Here, we randomly select two actors \textbf{\Sexpr{cleanNames[a1]}} and \textbf{\Sexpr{cleanNames[a2]}}. The shortest path between them contains \Sexpr{length(spath)-1} vertices: \Sexpr{common_movies}
