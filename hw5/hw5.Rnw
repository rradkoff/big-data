...
for ( k in 1:(length(spath)-1) ) { 
  ...
  common_movies[k] <- paste(names[spath[k]], "was in", names(casts)[intersect(m1, m2)],  
                            "with ", names[spath[k+1]])  
} 
@ 

Here, we randomly select two actors \textbf{\Sexpr{names[a1]}} and \textbf{\Sexpr{names[a2]}}. The shortest path between them contains \Sexpr{length(spath)-1} vertices: \Sexpr{common_movies}
