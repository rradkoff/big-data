...
Tables showing the most connected actors and busiest actors (by \# of movies completed) during the period are shown in \Vref{tab:conn,tab:busy} respectively.  Not surprisingly there is some overlap between the two; \textbf{\Sexpr{cleanActNames[most_connected]}} appears in both lists.
...
<<arules2, results='asis', cache=FALSE>>=
library(xtable)
actrules <- read.csv('actrules.csv', sep=';')
actrules <- data.frame(actrules, row.names='rules')
print(xtable(actrules, digits = c(0, 6, 4, 2), caption = 'Association Rules', 
             label = 'tab:actrules'))
@
...
\[ P(C|R) = \frac{1}{1+\exp(\alpha+\beta_{R})} = \Sexpr{conf} \]
...
\[ P(C,R) = \overline{D_{i,C} \cdot D_{i,R}} = \Sexpr{supp} \]
...
\[ \frac{P(C|R)}{P(C)} = \frac{1}{1+\exp(\alpha+\beta_{R})} \frac{1}{\overline{D_{i,C}}} = \Sexpr{lift} \]
